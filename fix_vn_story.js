...
// Replace the visual novel script
...
const visualNovelIdx = code.indexOf('visualNovel: {');
if (visualNovelIdx !== -1) {
    const cgIdx = code.indexOf('        cg: {', visualNovelIdx);
    const endIdx = code.indexOf('        init() {', visualNovelIdx);
    if (cgIdx !== -1 && endIdx !== -1) {
        code = code.substring(0, cgIdx) + newScriptData + '\n' + code.substring(endIdx);
