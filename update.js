...
const replacements = [
    { target: 'name: "TRPG / CRPG 🎲"', img: 'images/trpg.jpg' },
    { target: 'name: "RTS (Real-Time Strategy) - 실시간 전략 ⚔️"', img: 'images/rts.jpg' },
    { target: 'name: "턴제 전략 (Turn-based Strategy) 🎯"', img: 'images/턴제 전략.jpg' },
    { target: 'name: "MOBA 🏆"', img: 'images/moba.jpg' },
    { target: 'name: "배틀로얄 (Battle Royale) 🎪"', img: 'images/배틀로얄.jpg' },
    { target: 'name: "4X 및 대전략 (Grand Strategy) 👑"', img: 'images/4x.jpg' },
    ...
    { target: 'name: "게임 장르의 미래 트렌드 🚀"', img: 'images/vr.jpg' },
    { target: 'name: "MMORPG 🌍"', img: 'images/메이플.png' }
];

for (const {target, img} of replacements) {
    const escapedTarget = target.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // Regex matches the name line, optionally followed by an image line, or directly to def line
    const regex = new RegExp('(' + escapedTarget + '",\\s*)(?:image: "[^"]*",\\s*)?(def:)', 'g');
    ...
    if(!matched) console.log("Failed to match: " + target);
}
